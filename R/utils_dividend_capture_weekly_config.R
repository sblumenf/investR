...
#'
#' @param section Section name (e.g., "shared", "weekly_capture")
#' @param key Key name within section
#' @param fallback Fallback value if not found
#' @return Config value or fallback
#' @noRd
get_golem_config_value_weekly <- function(section, key, fallback = NULL) {
  tryCatch({
...
DIVIDEND_CAPTURE_CONFIG <- list(
  ...
  trading_days_per_year = get_golem_config_value_weekly("shared", "trading_days_per_year", 252),
  weeks_per_year = get_golem_config_value_weekly("weekly_capture", "weeks_per_year", 52),
  days_per_year = get_golem_config_value_weekly("shared", "days_per_year", 365),

  # Investment Parameters (from golem-config.yml)
  investment_amount = get_golem_config_value_weekly("weekly_capture", "investment_amount", 10000),

  # Data Validation (from golem-config.yml)
  min_dividend_events = get_golem_config_value_weekly("weekly_capture", "min_dividend_events", 5),

  # Rate Limiting (from golem-config.yml)
  rate_limit_seconds = get_golem_config_value_weekly("weekly_capture", "rate_limit_seconds", 2),

  # Parallel Processing (from golem-config.yml)
  max_workers = get_golem_config_value_weekly("weekly_capture", "max_workers", 4),
  min_workers = get_golem_config_value_weekly("shared", "min_workers", 1),
  max_workers_limit = get_golem_config_value_weekly("shared", "max_workers_limit", 10),
