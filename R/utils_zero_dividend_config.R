...
#' Get value from golem config with fallback (Zero Dividend)
#'
#' @param section Section name (e.g., "shared", "zero_dividend")
#' @param key Key name within section
#' @param fallback Fallback value if not found
#' @return Config value or fallback
#' @noRd
get_golem_config_value_zero_dividend <- function(section, key, fallback = NULL) {
  tryCatch({
...
ZERO_DIVIDEND_CONFIG <- list(
  ...
  strike_threshold_pct = get_golem_config_value_zero_dividend("zero_dividend", "strike_threshold_pct", 0.85),
  min_days = get_golem_config_value_zero_dividend("zero_dividend", "min_days", 45),
  max_days = get_golem_config_value_zero_dividend("zero_dividend", "max_days", 120),

  # Parallel Processing (from golem-config.yml)
  max_workers = get_golem_config_value_zero_dividend("zero_dividend", "max_workers", 10),

  # Data Validation (from golem-config.yml)
  min_option_bid = get_golem_config_value_zero_dividend("zero_dividend", "min_option_bid", 0.01),
  min_open_interest = get_golem_config_value_zero_dividend("zero_dividend", "min_open_interest", 10),
  max_stock_price = get_golem_config_value_zero_dividend("zero_dividend", "max_stock_price", 250),

  # Financial Constants (from golem-config.yml)
  shares_per_contract = get_golem_config_value_zero_dividend("zero_dividend", "shares_per_contract", 100),
  days_per_year = get_golem_config_value_zero_dividend("shared", "days_per_year", 365),

  # Date Ranges (from golem-config.yml)
  history_years = get_golem_config_value_zero_dividend("zero_dividend", "history_years", 5),
  cache_ttl_days = get_golem_config_value_zero_dividend("zero_dividend", "cache_ttl_days", 30),

  # Thresholds and Limits (from golem-config.yml)
  short_expiry_warning_days = get_golem_config_value_zero_dividend("zero_dividend", "short_expiry_warning_days", 14),
  negative_return_threshold = get_golem_config_value_zero_dividend("zero_dividend", "negative_return_threshold", 0),
  error_truncate_length = get_golem_config_value_zero_dividend("zero_dividend", "error_truncate_length", 50),

  # Output (from golem-config.yml)
  default_top_n = get_golem_config_value_zero_dividend("zero_dividend", "default_top_n", 10),
  output_dir = get_golem_config_value_zero_dividend("zero_dividend", "output_dir", "strategies")
)
